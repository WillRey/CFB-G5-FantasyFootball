...
// Pulls your live player CSV and returns:
//   teamNames       — Set of lowercased team display names (for filterLiveG6Games)
//   positionByPlayer — "firstname-lastname-team" → position string
//   dstTeamIds      — Set of ESPN team IDs for G6 teams (for DST filtering)
//   teamIdByName    — lowercased team name → ESPN team ID (populated later from scoreboard)
  ...
  async () => {
    ...
    for (const event of liveGames) {
      ...
      Object.values(boxscorePlayers).forEach(p => {
        ...
        if (!position) {
          // Unmatched players (OL, DL, etc. who show in some stat blocks) are
          // skipped — we only score players in your CSV
          return;
        }
      ...
      for (const comp of competitors) {
        ...
        // Only score D/STs for G6 teams (teams that appear in your CSV)
